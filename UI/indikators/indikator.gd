...
func start(num: float, pos: Vector3, multiplier: bool = false) -> void: #the new ready function
	if multiplier:
		...
		num = round(num)
		if num > 0: label.text = str('+ ', int(num)); modulate = Color.GREEN #increase of kredits
		elif num < 0: label.text = str('- ', abs(int(num))); modulate = Color.RED #decrease of kredits
...
func _process(_delta: float) -> void:
	global_position = get_viewport().get_camera_3d().unproject_position(target + (Vector3.UP / 4)) - (label.size / 2) #puts it over the target
