...
@onready var targetnode: PackedScene = preload("res://objekts/machines/arm/target/target.tscn")
@onready var avoidnode: PackedScene = preload("res://objekts/machines/arm/avoid/avoid.tscn")
@onready var prioritynode: PackedScene = preload("res://objekts/machines/arm/priority/priority.tscn")
...
@onready var settargetmat: StandardMaterial3D = preload("res://objekts/machines/arm/settarget.tres")
@onready var settargetidlemat: StandardMaterial3D = preload("res://objekts/machines/arm/settargetidle.tres")
...
var finaltarget: ArmTarget #the armtarget to go to
var finaltargetmachine: CollisionShape3D #the machine at the current final target
		...
		if finaltargetpos.length_squared() < 0.0001: #if there is no active target for the arm, get one!
			...
			for armtarget: ArmTarget in target_children: #iterates through all the targets
				...
				if is_instance_valid(machine):
					if not_viable_target(machine): 
						avaliable = false
					else:
						finaltargetmachine = machine #set the machine to use for later
						avaliable = true #everything else, then it is viable
				else: 
					avaliable = true
			...
			if is_instance_valid(targ_from_finalpos): #if there's a targeter at the position
				##the old system was beautiful, because if a machine was added to an empty viable target, it would check to make sure that the machine was viable before keeping it as a target since it's always looking for a machine there
				...
				if is_instance_valid(finaltargetmachine) and not_viable_target(finaltargetmachine): finaltargetpos = Vector3.ZERO
